...
class myHashMap<K,V> {
    ...
    public V remove(K key) {
        ...
        //find the bucket of the given key, go to the head of the chain
        int index = getBucketIndex(key);
        HashNode<K, V> head = bucket.get(index);
        V value = null;

        //traverse to find key, remove it, and reconnect the chain if there are more keys after the removed one
        if(head.key == key) {
            value = head.value;
    ...
    public V replace(K key, V val) {
        ...
        //find the bucket of the value to replace, go to head of bucket chain
        int index = getBucketIndex(key);
        HashNode<K, V> head = bucket.get(index);
        V value = null;

        //look for key, when we find it, swap the current value for the new one.
        if(head.key == key) {
            value = head.value;
            head.value = val;
        } else {
    ...
    public boolean replace(K key, V oldVal, V newVal) {
        ...
        //make sure that the replace has valid parameters, if not, return
        if (originalValue == null ||
                (! originalValue.equals(oldVal)) ) {
            return false;
        }

        //if it passes, call the function
        replace(key, newVal);
